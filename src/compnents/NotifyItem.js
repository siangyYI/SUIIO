...
class NotifyItem extends Component {
  render() {
    ...
    return (
      <>
        <a
          href={`/${notufy.type}/detail?ID=${notufy.objectID}`}
          className="cardHover"
        >
          <div className="ncardline py-3 ">
            <div
              className="nfont ml-4  Col"
            ...
            <div
              className="ml-4  Col"
              style={{ textAlign: "left", color: "#545149" }}
            >
              {[month, date.toString().padStart(2, "0")].join("-")}{" "}
              {[hours, minte.toString().padStart(2, "0")].join(":")}
            </div>
          </div>
        </a>
      </>
